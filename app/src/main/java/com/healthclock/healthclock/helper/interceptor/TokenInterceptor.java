package com.healthclock.healthclock.helper.interceptor;
...
public class TokenInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        int status = Integer.parseInt(extrasJson.optString("status")); //根据后台返回数据执行修改

//        if (response shows expired token){//根据和服务端的约定判断token过期
        if (status == 401){  //假设服务端返回码401为token过期
